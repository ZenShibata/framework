    ...
    public async parseConditions(guildId: string | undefined, channelId: string | null, user: { id: string } | null | undefined, client: { id: string } | null | undefined, context: { permissions: PermissionsBitField }): Promise<Result<unknown, UserError>> {
        if (guildId && user?.id) {
            ...
            if (client && voiceState?.channelId) {
                const channel = await this.container.client.resolveChannel({ id: voiceState.channelId, guildId, force: true, cache: true });
                const member = await this.container.client.resolveMember({ id: client.id, guildId, force: true, cache: true });
                if (channel && member) {
